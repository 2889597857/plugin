...
const setRef = (rawRef, oldRawRef, parentComponent, parentSuspense, vnode) => {
	...
	if (isString(ref)) {
		...
		callWithErrorHandling(ref, parentComponent, 12 /* FUNCTION_REF */, [value, refs])
	} else {
...
const normalizeRef = ({ ref }) => {
	return ref != null ? (isArray(ref) ? ref : { i: currentRenderingInstance, r: ref }) : null
}
...
function useCssVars(getter, scoped = false) {
	...
	const prefix =
		scoped && instance.type.__scopeId ? `${instance.type.__scopeId.replace(/^data-v-/, '')}-` : ``
				...
				if (returnType !== 2 /* HAS_RUNTIME_CONSTANT */) {
					for (let i = 0; i < node.props.length; i++) {
						...
						if (
							...
							p.name === 'bind' &&
							p.exp &&
							(p.exp.type === 8 /* COMPOUND_EXPRESSION */ || p.exp.isRuntimeConstant)
						) {
							returnType = 2 /* HAS_RUNTIME_CONSTANT */
						}
					}
...
const transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
	...
	return processFor(node, dir, context, forNode => {
		...
		const isStableFragment =
			forNode.source.type === 4 /* SIMPLE_EXPRESSION */ && forNode.source.isConstant
